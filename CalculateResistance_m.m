...
%Calculating the resistance of all Äste
R_e=zeros(n-1,1);
z_a=zeros(n-1,1);
lambda = zeros((n-1),1);
...
Z_A=zeros(n-1); % To calculate the resistance of the Sehnen a resistance-Matrix of Äste is needed
for i=1:(n-1)
        Z_A(i,i) = z_a(i);
end

Zaehler = B_A*Z_A*m_old(1:(n-1))./rho(1:(n-1))+p_Q(1:(n-1)); %fehlen Druckquellen und gesteuerte Druckabfälle: -B_A*P_q(1:(n-1)) +B_A*P_st_old(1:(n-1))
